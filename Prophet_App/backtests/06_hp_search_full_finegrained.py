"""
Grid search de hiperparametros de Prophet con el UNIVERSO COMPLETO (89 tickers
x 10 cortes), ventana fija en "1 año". Solo se varian los parametros que
mostraron efecto real o ambiguo en la busqueda reducida anterior:
  - changepoint_prior_scale: resolucion fina alrededor de la zona buena (0.03-0.15)
  - seasonality_mode: multiplicative vs additive (resultado ambiguo a validar)
yearly_seasonality queda fijo en True (ya establecido como ganador en este
rango de changepoint_prior_scale).
"""
...
HP_GRID = {
    "changepoint_prior_scale": [0.03, 0.05, 0.07, 0.10, 0.15],
    "seasonality_mode":        ["multiplicative", "additive"],
}
COMBOS = [
    {"changepoint_prior_scale": cps, "seasonality_mode": sm, "yearly_seasonality": True}
...
for ticker, cl in series.items():
    for cutoff in CUTOFFS:
        ...
        for combo in COMBOS:
            ...
            combo_key = f"cps={combo['changepoint_prior_scale']}|mode={combo['seasonality_mode']}"
            try:
                m = Prophet(
                    interval_width=0.95,
...
out_path = r"results\hp_search_full_results.csv"
